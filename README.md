...
<p align="center"><strong>A Twitch chat bot that actually remembers your regulars — names, inside jokes, the callback from three weeks ago. Runs with Gemini in the cloud or Qwen/Gemma locally.</strong></p>
...
4. A reply is generated by Gemini in the cloud, or by a local model (Qwen, Gemma — via Ollama or any OpenAI-compatible server) running on your own machine.
...
Nothing passes through a shared backend. Cloud calls travel from your machine directly to Gemini with your own key; local-model calls never leave your machine at all.
...
The runtime expects credentials supplied by the toolset pairing flow (stored locally). An AI setup is required — a free Gemini key, or a local model via Ollama. The setup wizard verifies whichever you pick end-to-end before the bot goes live. Environment equivalents: `BOT_LLM_API_KEY` (cloud) or `BOT_LLM_BASE_URL` + `BOT_LLM_LOCAL_MODEL` (local). Without a working AI setup the reply engine starts in disabled mode.
...
Local-first: all chat memory lives in a SQLite file on the broadcaster's machine. LLM calls go from your machine directly to Gemini — or stay on your machine entirely when running Qwen or Gemma locally. No shared server ever holds your chat data. Credentials are not stored in plaintext on disk.
